package top.gyj.computerstore.controller;
...
import top.gyj.computerstore.config.AlipayConfig;
...
public class AliPayController {
    ...
    public String goAlipay(String oid, String totalPrice, HttpServletRequest request) throws Exception {
        ...
        // 确保session中有uid，如果没有则尝试从已登录用户中获取
        Integer uid = (Integer) request.getSession().getAttribute("uid");
        if (uid == null) {
            // 记录错误日志
            log.error("用户未登录或session中未找到uid，支付可能无法完成");
        }
    ...
    public void alipayReturnNotice(HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        if (signVerified) {
            ...
            // 如果session中没有uid，尝试从passback_params中获取
            if (uid == null && request.getParameter("passback_params") != null) {
                String passback_params = new String(request.getParameter("passback_params").getBytes("ISO-8859-1"),
                        "UTF-8");
                // 对传回的数据进行解码
                String uidStr = URLDecoder.decode(passback_params, "UTF-8");
            ...
            if (uid != null) {
                try {
                    orderService.updateOrderStatusByOid(oid, uid, 1);
                    log.info("同步回调成功更新订单状态，订单号：{}，用户ID：{}", oid, uid);
                } catch (Exception e) {
                    log.error("同步回调更新订单状态失败：" + e.getMessage(), e);
                }
            } else {
                log.error("无法获取用户ID，订单状态更新失败，订单号：{}", oid);
            }
